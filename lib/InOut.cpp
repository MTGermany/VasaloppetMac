...
int InOut::getNumberOfDataLines(string fname){
  return getNumberOfDataLines(fname.c_str());
}
...
//###################################################################
//MT2023-02: get_col with vector argument (first one vector of doubles)



//###################################################################
//!!! MT2024-11: double get_col with changed interface  and string fname)
// for some annoying reason templates for any primitive types not usable
// (no useful info on the web)
//###################################################################


vector<double> InOut::get_col(string fname, int col){
  //cout<<"\nin vector<double> InOut::get_col: col="<<col<<endl;
  ifstream  infile (fname, ios::in);
  if( !infile)
  {
    cerr << "Error opening file " << fname << " for reading" << endl;
  ...
  vector<double> data;
  int i=0;
  string proforma; // read any other types in previous lines
  ...
  for (string line; getline(infile, line); ){  
    if(is_data_line(line)){
      //cout<<"vector<double> get_col: line="<<line<<endl;
      prepare_line(line);
      istringstream linestream (line);
      for (int icol=1; icol<col; icol++){linestream >> proforma;}
...
//###################################################################
//!!! MT2024-11: int get_col with changed interface  and string fname)
// for some annoying reason templates for any primitive types not usable
// (no useful info on the web)
//###################################################################


vector<int> InOut::get_intcol(string fname, int col){
  //cout<<"\nin vector<int> InOut::get_col: col="<<col<<endl;
  ifstream  infile (fname, ios::in);
  if( !infile)
  {
    cerr << "Error opening file " << fname << " for reading" << endl;
  ...
  int i=0;
  string proforma; // read any other types in previous lines
  int oneElement; // used for push_back
  ...
  if(errcnt>0){cerr<<"InOut.get_intcol: file="<<fname<<" col="<<col
		   <<": WARNING: not all data were double! Set to -999 in "
		   <<errcnt<<" cases!"<<endl;
  }
  return data;
...
//###################################################################
//!!! MT2024-11: string get_col with changed interface  and string fname)
// for some annoying reason templates for any primitive types not usable
// (no useful info on the web)
//###################################################################

vector<string> InOut::get_stringcol(string fname, int col){
  ifstream  infile (fname, ios::in);
  if( !infile)
  {
    cerr << "Error opening file " << fname << " for reading" << endl;
  ...
  vector<string> data;
  int i=0;
  string oneElement; // used for push_back
  
  for (string line; getline(infile, line); ){
    if(is_data_line(line)){
      prepare_line(line);
      istringstream linestream (line);
      for (int icol=1; icol<col; icol++){linestream >> oneElement;}
      linestream>>oneElement;
...
//##########################################################
//##########################################################
// get_col with vector of doubles
...
// #######################################################
// get_col with vector of strings
...
void InOut::get_col(const char* fname, int col,  int & n_array,
  ...
  while ( (infile.getline(line,LINEMAX))) {
    ...
    if(is_data_line(line)){
	  ...
	  errcnt++;
          oneElement="NA";
	  if(false){
	    cerr<<"error while parsing for string ... i="<<i
	        <<" col="<<col<<" datum="<<oneElement
	        <<" line=" <<line<<endl;
	  }
      }
  ...
  n_array = i;
  if(true){
    cout << "get_col(vector<string>&): " << "fname = "<<fname 
         <<" col="<<col<<" n_array = " << n_array << endl;
  }
  if(errcnt>0){cerr<<"InOut.get_col: file="<<fname<<" col="<<col
		   <<": WARNING: not all data were available! Set to \"NA\" in "
		   <<errcnt<<" cases!"<<endl;
  }
}
